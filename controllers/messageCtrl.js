...
const messageCtrl = {
  createMessage: async (req, res) => {
    try {
      ...
      // update newConversation, which recipients array is [sender, recipient] or [recipient, sender]
  ...
  getMessages: async (req, res) => {
    try {
      const features = new APIFeatures(
        //message have sender or recipient as user
        Messages.find({
          $or: [
  ...
  deleteConversation: async (req, res) => {
    try {
      const newConver = await Conversations.findOneAndDelete({
        $or: [
          { recipient: [req.user._id, req.params.id] },
          { recipient: [req.params.id, req.user._id] },
        ],
      });
